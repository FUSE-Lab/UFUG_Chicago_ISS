...
# Custom correlation function to reorder labels
# 박스플롯 순서를 "Invaded" -> "Non-invaded"로 변경
FinalTable_filtered$invaded_ch <- factor(FinalTable_filtered$invaded_ch,
                                         levels = c("Invaded", "Non-invaded"))

# ggpairs 실행
pm <- ggpairs(FinalTable_filtered, 
              columns = c(5, 17, 27, 29, 30, 3), 
              ggplot2::aes(colour = (invaded_ch), alpha = 0.2),
              columnLabels = c("Density", "MoM_CH", "VCI", "VAI", "AI_subcanopy", "NDVI"),
              upper = list(continuous = wrap("cor", size = 5)),
              diag = list(continuous = wrap("box_no_facet", alpha = 0.5)), 
              lower = list(continuous = "smooth")) + 
  scale_colour_manual(values = c("#f56455", "#15134b")) + # "Invaded" -> 빨간색
  scale_fill_manual(values = c("#f56455", "#15134b")) + 
  theme_bw() + 
  theme(strip.text.x = element_text(size = 15),
        strip.text.y = element_text(size = 15),
        axis.text = element_text(size = 13))
